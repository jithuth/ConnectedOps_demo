...
namespace ConnectedOps.Web.Pages.Compliance.Assets;
...
public sealed class DetailsModel : PageModel
{
    ...
    public List<SelectListItem> AssetRequirements { get; private set; } = [];
    ...
    public async Task<IActionResult> OnPostAddRecordAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await _recordService.CreateAsync(new CreateComplianceRecordRequest(
                ...
                SubjectType: ComplianceSubjectType.Asset,
                AssetId: Id,
            ...
            return RedirectToPage("/Compliance/Assets/Details", new { id = Id });
        }
        catch (Exception ex)
        {
    ...
    {
        Evaluation = await _evaluationService.EvaluateAssetAsync(Id, cancellationToken);
        Records = await _recordService.GetSubjectRecordsAsync(ComplianceSubjectType.Asset, Id, cancellationToken);

        var activeReqs = await _requirementService.GetAllActiveAsync(cancellationToken);
        AssetRequirements = activeReqs
            .Where(r => r.AppliesTo == ComplianceSubjectType.Asset)
