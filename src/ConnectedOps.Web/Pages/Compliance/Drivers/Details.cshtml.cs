...
namespace ConnectedOps.Web.Pages.Compliance.Drivers;
...
public sealed class DetailsModel : PageModel
{
    ...
    public List<SelectListItem> DriverRequirements { get; private set; } = [];
    ...
    public async Task<IActionResult> OnPostAddRecordAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await _recordService.CreateAsync(new CreateComplianceRecordRequest(
                ...
                SubjectType: ComplianceSubjectType.Driver,
                DriverId: Id,
            ...
            return RedirectToPage("/Compliance/Drivers/Details", new { id = Id });
        }
        catch (Exception ex)
        {
    ...
    {
        Evaluation = await _evaluationService.EvaluateDriverAsync(Id, cancellationToken);
        Records = await _recordService.GetSubjectRecordsAsync(ComplianceSubjectType.Driver, Id, cancellationToken);

        var activeReqs = await _requirementService.GetAllActiveAsync(cancellationToken);
        DriverRequirements = activeReqs
            .Where(r => r.AppliesTo == ComplianceSubjectType.Driver)
