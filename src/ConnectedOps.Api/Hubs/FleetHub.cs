...
namespace ConnectedOps.Api.Hubs;
...
public sealed class FleetHub : Hub
{
    ...
    public async Task JoinTenantGroup(string tenantId)
    {
        if (Guid.TryParse(tenantId, out _))
        {
            ...
            _logger.LogInformation("API Connection {ConnectionId} joined tenant group {TenantGroup}", Context.ConnectionId, $"tenant-{tenantId}");
        }
    }
